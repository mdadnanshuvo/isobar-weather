...
This app runs two ways from the same shared logic (`netlify/functions/_shared.mjs`):
locally via a normal Express server (`server.js`), and on Netlify via
serverless Functions (`netlify/functions/*.mjs`). Netlify doesn't run a
persistent Node server, so `server.js` is dev-only — production uses the
Functions.

**Build settings** (Site settings → Build & deploy, or the deploy-config
screen when you first connect the repo):
...
These are also encoded in `netlify.toml`, so if Netlify reads that file
automatically you may not need to fill them in by hand.

**Environment variable** — Site settings → Environment variables → add
`WEATHERAI_API_KEY` with your key. Do **not** put it in `netlify.toml` or
commit a `.env` with the real value; Functions read it from
`process.env.WEATHERAI_API_KEY` at request time.

After deploying, `public/app.js`'s calls to `/api/weather`, `/api/geo`, and
`/api/usage` get rewritten by the `[[redirects]]` rule in `netlify.toml` to
`/.netlify/functions/weather`, `/.netlify/functions/geo`, and
`/.netlify/functions/usage` — no frontend code changes needed between local
dev and production.

If you see `Unexpected token '<' ... not valid JSON` in the UI, it means
`/api/*` isn't reaching a function at all (usually a missing Functions
directory setting or a fresh deploy that hasn't picked up `netlify.toml`
yet) — the fetch is landing on Netlify's HTML 404 page instead.

## What I'd add next, given more time

- A durable shared cache (Redis, or Netlify Blobs) instead of the
  best-effort in-memory one, so quota savings hold across cold starts.
...
netlify/functions/_shared.mjs Shared WeatherAI-calling logic (cache, retries, error mapping)
netlify/functions/weather.mjs Production /api/weather (Netlify Function)
netlify/functions/geo.mjs     Production /api/geo
netlify/functions/usage.mjs   Production /api/usage
netlify.toml                  Build settings + /api/* redirect to Functions
public/index.html             Markup
public/style.css              Design tokens + layout
public/app.js                 Location resolution, rendering, API-shape normalization
.env.example                  Copy to .env with your own key (local dev only)
```
